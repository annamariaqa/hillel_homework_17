...
def get_address():
    ...
    addrParam = ['street address', 'landmark', 'area', 'city', 'state', 'pincode']
    full_addr = []
    ...
    try:
        with open(full_path('data.csv'), 'r') as csvfile:
            ...
            missing_columns = [param for param in addrParam if param not in headers]
            if missing_columns:
                raise ValueError(f"Missing columns in CSV: {missing_columns}")

            data_by_param = {param: [] for param in addrParam}
            for row in reader:
                for param in addrParam:
                    if param in headers:
                        index = headers.index(param)
                        if len(row) > index and row[index].strip():
                            data_by_param[param].append(row[index].strip())

            for param in addrParam:
                if data_by_param[param]:
                    full_addr.append(choice(data_by_param[param]))
                else:
                    full_addr.append(f"Missing {param}")

        full_addr = dict(zip(addrParam, full_addr))
        return full_addr

    except Exception as e:
        print(f"Error reading address data: {e}")
        return {}
